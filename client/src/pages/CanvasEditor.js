...
const AdvancedCanvasEditor = () => {
    ...
    const handleSelect = (e) => {
        if (e.selected?.length === 1) {
            ...
            const objects = e.selected.map(item => objectMapRef.current[item.id]);
   ...
   useEffect(() => {
    ...
    const handlePathCreated = (e) => {
        ...
        saveToHistory(); // ✅ Needed to enable undo
    };
   ...
   const handleMouseDown = (e) => {
    ...
    // ✅ Ensure this logic is outside of switch
    if (shape) {
        canvas.add(shape);
    ...
    setIsDrawing(true);
        ...
        function createStarPoints(points, outerRadius, innerRadius) {
            ...
            for (let i = 0; i < 2 * points; i++) {
                ...
                const a = i * angle - Math.PI / 2; // rotate so the first point is at the top
                starPoints.push({
                    x: Math.cos(a) * r,
                    y: Math.sin(a) * r,
                });
